...
# ///
"""No-MCP fallback search for scholarflow-literature-search (OpenAlex, stdlib only).
...
`scholarflow-citation-review/scripts/citation_review.py` already does for CrossRef: a single
self-contained file hitting a public HTTP API with no third-party dependencies.

Source: OpenAlex (https://openalex.org) 鈥?free, no API key. OpenAlex indexes
...
.ris/.bib/.enw citation files 鈥?together they make the search鈫抏xport workflow
