#!/usr/bin/env python3
"""Create a PaperSpine configuration from a guided terminal intake."""
...
def build_config() -> PaperSpineConfig:
    print("PaperSpine Intake")
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=Path("paper_spine_config.json"))
    parser.add_argument("--non-interactive", action="store_true")
    args = parser.parse_args()
    config = PaperSpineConfig() if args.non_interactive else build_config()
    data = asdict(config)
