#!/usr/bin/env python3
"""ScholarFlow Manuscript Studio Translation Guard - verify translation_zh/ completeness and quality.
...
from _manuscript_studio_utils import markdown_tables, read_text, table_rows
...
TRANSLATION_COMMON = [
    ...
    "scholarflow_manuscript_config.zh.md",
    "source_map.zh.md",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Verify ScholarFlow Manuscript Studio translation_zh/ completeness and quality.")
    ...
    for f in missing:
        ...
        findings.append(TranslationFinding(
            ...
            fix=f"Translate `{en_name}` into `translation_zh/{f}`. "
                f"Use `manuscript-studio-translate` to produce this file.",
            teaching=f"Every intermediate artifact needs a Chinese counterpart. "
                      f"`{en_name}` is a required ScholarFlow Manuscript Studio artifact - its translation "
                      f"is not optional.",
        ))
