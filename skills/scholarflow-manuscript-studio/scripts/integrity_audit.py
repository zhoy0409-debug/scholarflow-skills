#!/usr/bin/env python3
"""ScholarFlow Manuscript Studio Integrity Audit 鈥?teaching-quality checkpoint before LaTeX.
...
the user and downstream ScholarFlow Manuscript Studio agents so they can reason about what to fix
and why.
...
from _manuscript_studio_utils import (
    is_separator_row,
    markdown_tables,
    read_text,
    split_table_line,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the ScholarFlow Manuscript Studio integrity audit (teaching mode).")
...
def load_config(out_dir: Path) -> dict:
    config_path = out_dir / "scholarflow_manuscript_config.json"
...
ARTIFACT_OWNERS: dict[str, str] = {
    "scholarflow_manuscript_config.json": "manuscript-studio-intake",
    "research_dossier.md": "manuscript-studio-research",
    "exemplar_learning_dossier.md": "manuscript-studio-research",
    "citation_support_bank.md": "manuscript-studio-citation",
    "confirmed_motivation.md": "user confirmation (after manuscript-studio-research)",
    "section_blueprints.md": "manuscript-studio-rewrite / manuscript-studio-build",
    "writing_rationale_matrix.md": "manuscript-studio-rewrite / manuscript-studio-build",
    "original_logic_map.md": "manuscript-studio-rewrite",
    "evidence_bank.md": "manuscript-studio-rewrite / manuscript-studio-build",
    "rewrite_matrix.md": "manuscript-studio-rewrite",
    "logic_transfer_audit.md": "manuscript-studio-rewrite",
    "material_inventory.md": "manuscript-studio-build",
}
...
def audit_artifacts(out_dir: Path, config: dict) -> AuditDimension:
    ...
    required = [
        "scholarflow_manuscript_config.json", "research_dossier.md",
    ...
    for i, artifact in enumerate(required, start=1):
        ...
        if not path.exists():
            ...
            dim.findings.append(AuditFinding(
                ...
                teaching_note=f"Each ScholarFlow Manuscript Studio artifact is a checkpoint. When one is missing, it usually means "
...
def audit_reasoning_depth(out_dir: Path, _config: dict) -> AuditDimension:
    ...
    if not matrix_path.exists():
        dim.findings.append(AuditFinding(
            ...
            root_cause="manuscript-studio-rewrite or manuscript-studio-build did not produce the rationale matrix",
