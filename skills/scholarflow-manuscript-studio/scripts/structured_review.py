#!/usr/bin/env python3
"""Structured Peer Review for ScholarFlow Manuscript Studio manuscripts.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Structured peer review for ScholarFlow Manuscript Studio manuscripts.")
...
def generate_structured_review(out_dir: Path, manuscript_path: Path) -> StructuredReviewReport:
    ...
    for cr in contrib_rows[:3]:
        ...
        contribution.findings.append(ReviewFinding(
            ...
            teaching_note="A contribution is not what you did; it is what the community gains. "
    ...
    for reviewer in report.reviewers:
        ...
        for f in reviewer.findings:
            if f.severity == "OBSERVATION":
                continue
            icon = "[CRITICAL]" if f.severity == "CRITICAL" else "[MAJOR]" if f.severity == "MAJOR" else "[MINOR]"
