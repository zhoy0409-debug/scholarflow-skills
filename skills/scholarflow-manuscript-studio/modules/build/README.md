# ScholarFlow Manuscript Studio Build From Materials
...
section blueprint, and writing rationale matrix logic as `manuscript-studio-rewrite`.
It is not a separate shortcut.

## Required Inputs

- `paper_rewriting_output/scholarflow_manuscript_config.json`
- `materials_dir`
...
`manuscript-studio-research` and ask the user to confirm the motivation after seeing
research-grounded options.

If `citation_support_bank.md` is missing or shallow, return to
`manuscript-studio-citation`. From-zero writing still needs literature support for
...
- `paper_rewriting_output/translation_zh/` when `translation_package` is `zh` (via `manuscript-studio-translate`)
...
If `scholarflow_manuscript_config.json` has `humanize_tier` set to `light`, `medium`, or
`heavy`, route to `manuscript-studio-humanize` before writing.  The humanize skill
...
- Before routing through `manuscript-studio-latex`, route through `manuscript-studio-audit` for integrity and structured review. Only proceed when all required dimensions pass.
- Always finish by routing through `manuscript-studio-latex`. A Markdown draft is not
