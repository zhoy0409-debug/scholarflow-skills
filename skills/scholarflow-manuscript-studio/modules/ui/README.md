# ScholarFlow Manuscript Studio UI

Use this branch whenever `scholarflow_manuscript_config.json` is missing, incomplete, or
the user asks to configure ScholarFlow Manuscript Studio interactively.

## Required Behavior

The supported interaction is a real terminal window launched by
`scripts/launch_manuscript_studio_ui.ps1`. Do not run `input()`-based Python inside a
hidden tool surface when the host cannot expose stdin.

In Claude Code, `/manuscript-studio` must call this branch automatically when config is
missing. `/manuscript-studio-legacy` is only a manual fallback.

In Codex, use the same launcher when PowerShell is available:

```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -File scripts/launch_manuscript_studio_ui.ps1 -OutputDir paper_rewriting_output
```
...
paper_rewriting_output/scholarflow_manuscript_config.md
```
