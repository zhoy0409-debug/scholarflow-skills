# ScholarFlow Manuscript Studio Humanize

**Self-contained skill** — can be used standalone on any academic text, or as
part of the ScholarFlow Manuscript Studio suite.  Applies tier-specific writing constraints based
on real AIGC detection dimensions and produces a teaching matrix that documents
every change.

## When To Use

- In ScholarFlow Manuscript Studio: auto-invoked by rewrite/build when `humanize_tier` is not `none`
- Standalone: invoke directly on any academic text that needs AI detection reduction

## Configuration

Read `scholarflow_manuscript_config.json` for `humanize_tier` (none/light/medium/heavy)
...
In ScholarFlow Manuscript Studio: called internally by rewrite and build.  Standalone: place this
