# ScholarFlow Manuscript Studio Rewrite
...
`manuscript-studio-citation` before final writing. Introduction, background, related
...
- `translation_zh/` package when `translation_package` is `zh` (via `manuscript-studio-translate`)
...
Before routing output to `manuscript-studio-latex`, run:

Route the completed package through `manuscript-studio-audit` for integrity and structured review before final delivery.
