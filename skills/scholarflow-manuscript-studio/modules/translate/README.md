# ScholarFlow Manuscript Studio Translate
...
Read `scholarflow_manuscript_config.json` to determine `workflow`.  List every file that
...
|---|---|
| `scholarflow_manuscript_config.json` | `scholarflow_manuscript_config.zh.md` |
...
This branch is called by the orchestrator after `manuscript-studio-latex` and before
`manuscript-studio-audit`.  The orchestrator's Non-Negotiable Route step 14 requires
