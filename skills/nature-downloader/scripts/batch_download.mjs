...
async function handleWosAuthPreference(proxy, target) {
  ...
  if (!/AUTH_PREFERENCE_ERROR|authentication preference|Authentication Preference/i.test(marker)) {
    return false;
  }
  const clicked = await evalJs(
    proxy,
    target,
    `(()=>{const r=document.querySelector('#radio-shibboleth,input[value="shibboleth"]');if(r)r.click();const b=[...document.querySelectorAll('button,a,input[type=button],input[type=submit]')].find(e=>/(Continue|Proceed|Next|Submit)/i.test(e.innerText||e.value||''));if(b)b.click();return !!b;})()`
  ).catch(() => false);
  if (clicked) {
