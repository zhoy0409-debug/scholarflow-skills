...
Open this reference when running `scripts/citation_review.py` or when the input is long enough to need batching. It expands step 3 of the workflow.

## Running the script

Start with `scripts/citation_review.py` when internet access is available:

```bash
python scripts/citation_review.py \
  --text "PASTE MANUSCRIPT TEXT HERE" \
  --scope cns \
  --outdir /tmp/scholarflow-citation-review \
  --format enw \
  --with-artifacts
```
