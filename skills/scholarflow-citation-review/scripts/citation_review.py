...
USER_AGENT = "codex-scholarflow-citation-review/1.0 (mailto:unknown@example.com)"
...
def crossref_headers(mailto: str | None = None) -> dict[str, str]:
    return {"User-Agent": USER_AGENT if not mailto else f"codex-scholarflow-citation-review/1.0 (mailto:{mailto})"}
...
def write_report(
    ...
    lines = [
        "# ScholarFlow Citation Review Report",
        "",
        "## Search Scope",
        "",
        f"- Scope: `{scope}`",
...
<head>
  ...
  <title>ScholarFlow Citation Review Browser</title>
  <style>
    :root {{
      --ink: #1f1f1f;
      --muted: #5f6368;
...
<body>
  <header>
    <h1>ScholarFlow Citation Review Browser</h1>
