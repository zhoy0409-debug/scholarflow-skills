---
name: zotero-lit-fetch
description: Fetch and organize literature metadata into Zotero-oriented workflows across English and Chinese-language scholarly databases while respecting authorization and access limits.
---

# Zotero Lit Fetch — Router

把检索结果连同 PDF 抓进 Zotero

**Do not answer from memory, and do not answer from this file.**
The actual logic lives in the files below. This router only decides which to load.
Loading them is not optional.

## 1. Load what this request needs

**References** —— 按需读，不要一次全读：

- `references/databases.md`
- `references/setup.md`
- `references/zotero-connector.md`

**Scripts** —— 这些是真的要跑的，不是拿来读的：

- `scripts/oa_pdf.py`
- `scripts/zotero_check.py`
- `scripts/zotero_save.py`

## 2. Do the work

先说清你**选了哪条路**、**用了哪些文件** —— 一行就够，让用户能便宜地纠正你。

缺必要输入时，写占位符并列在 `Assumptions or missing inputs:` 下面 —— **不要编。**

## 3. Check before delivering

说清你假设了什么、什么没能核实、什么还开着。
