#!/usr/bin/env python3
"""Read-only QA helper for review manuscripts in DOCX format."""
...
HEADING_RE = re.compile(r"^([A-Za-z][A-Za-z0-9 ,:;()/-]{2,80}|[1-9]\d?(?:\.\d+)*\s+\S+)")
CAPTION_RE = re.compile(r"^(Figure\s+\d+|Fig\.\s*\d+|Table\s+\d+|Tab\.\s*\d+)")
FORBIDDEN = [
...
def has_child(element, local_name: str) -> bool:
    return any(child.tag.endswith("}" + local_name) for child in element)
    ...
    for idx, shape in enumerate(doc.inline_shapes, start=1):
        ...
        if Image is not None:
            ...
            if rid and rid in doc.part.related_parts:
                try:
                    with Image.open(BytesIO(doc.part.related_parts[rid].blob)) as img:
                        pixels = img.size
...
def main() -> int:
    ...
    print(f"  cited but missing from reference list: {sorted(cited - set(ref_nums)) or 'none'}")
    print(f"  listed but not cited in body/tables: {sorted(set(ref_nums) - cited) or 'none'}")
    print("\n[FORBIDDEN / PROCESS WORDING]")
    hits = [(word, sum(text.lower().count(word.lower()) for text in texts)) for word in FORBIDDEN]
    hits = [(word, count) for word, count in hits if count]
    if hits:
    ...
    for idx, pixels, width_in, height_in, dpi in image_report(doc):
        dpi_text = "unknown" if dpi is None else f"{dpi[0]:.0f} x {dpi[1]:.0f}"
        print(f"  image {idx}: pixels={pixels or 'unknown'} size={width_in:.2f} x {height_in:.2f} in dpi={dpi_text}")
    return 0
