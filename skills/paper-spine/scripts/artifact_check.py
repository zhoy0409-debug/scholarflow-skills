#!/usr/bin/env python3
"""Check whether required PaperSpine workflow artifacts exist and look usable."""
...
def audit(root: Path, min_chars: int = 120) -> list[ArtifactFinding]:
    ...
    config_path = root / "paper_spine_config.json"
    if config_path.exists():
        try:
            json.loads(config_path.read_text(encoding="utf-8"))
        except json.JSONDecodeError as exc:
            findings.append(ArtifactFinding("ERROR", "paper_spine_config.json", f"invalid JSON: {exc}"))
    return findings
